...
*  It is recommended to `enter 0, 0` and `leave` a function
...
*  `EBP` must not be modified *if* `enter` is used (can be `push`ed, modified and `pop`ed instead)
*  `ESP` must not be modified *unless* `enter`ed (can be `mov`ed into `EBP`, modified and `mov`ed back instead)
