...
if exists('g:loaded_syntastic_asm_nasm_checker')
    finish
endif
let g:loaded_syntastic_asm_nasm_checker = 1

let s:save_cpo = &cpo
set cpo&vim

function! SyntaxCheckers_asm_nasm_GetLocList() dict
    let buf = bufnr('')
    let makeprg = self.makeprgBuild({
        \ 'args_after': '-f elf -X gnu' .
