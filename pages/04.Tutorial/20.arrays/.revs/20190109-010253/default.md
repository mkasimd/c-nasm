...
Basically, we have been using arrays quite a few times now (e,g, in the [Hello World](../hello-world) program) but also pointers like `char * string` (e.g. in the [Print Functions](../print-functions)). Both `char* string` and the `char string[]` array are **similar, yet different** in C. So an `int arr[]` array is not much different from `char string[]` except for it not being necessarily null terminated.
