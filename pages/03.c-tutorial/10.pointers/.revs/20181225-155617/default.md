...
```
!!! NOTE: `a[i]`is equivalent to `*(a_pt + i)` is equivalent to `*(i + a_pt)` is equivalent to `i[a_pt]`. It is not a standard use, but addressing an array element is possible in all of those equivalent ways. As to for more on the reason, check Arrays in ASM and the commutativity in the ring (ℤ, +, \*).
